	...
	public function render() {
		...
		<div id="debug-bar-rewrite-resting">
			...
			<?php if ( ! empty( $this->summary['status'] ) ) : ?>
				<h2><span><?php _e( 'Status:', 'debug-bar-remote-requests' ); ?></span> <?php echo esc_html( $this->summary['status'] ) ?></h2>
			<?php endif;

			if ( ! empty( $this->summary['error_count'] ) ) : ?>
				<h2><span><?php _e( 'Total Errors:', 'debug-bar-remote-requests' ); ?></span> <?php echo absint( $this->summary['error_count'] ) ?></h2>
			<?php endif; ?>
			...
			<h3><?php _e( 'Errors', 'rewrite-testing' ); ?></h3>
